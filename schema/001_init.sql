...
create table tbl_physio_progress_notes (
  ...
  evaluation             text,
  completion             text,
  therapist_id           bigint references tbl_staff (id),
  created_at             timestamptz not null default now(),
  check (
...
create table tbl_fall_incidents (
  ...
  witness             text,
  reported_by         bigint references tbl_staff (id),
  created_at          timestamptz not null default now()
);
...
create table tbl_products (
  ...
  status          text not null default 'ACTIVE' check (status in ('ACTIVE','INACTIVE')),
  registered_by   bigint references tbl_staff (id),
  created_at      timestamptz not null default now(),
  updated_at      timestamptz not null default now()
);
...
create table tbl_stock_transfers (
  ...
  dispatched_by             bigint references tbl_staff (id),
  dispatched_at             timestamptz not null default now(),
  confirmed_by              bigint references tbl_staff (id),
...
create table tbl_stock_request_details (
  ...
  ordered_by                  bigint references tbl_staff (id),
  ordered_date                date,
