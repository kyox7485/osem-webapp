...
export function PageHeaderSlot() {
  ...
  return (
    <div className="min-w-0 flex-1">
      {content && (
        <>
          <h1 className="truncate text-lg font-bold tracking-tight text-gray-900 dark:text-gray-100">{content.title}</h1>
          {content.description && <p className="mt-0.5 truncate text-xs text-gray-500 dark:text-gray-400">{content.description}</p>}
        </>
      )}
    </div>
  );
