...
export function TabRow({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`inline-flex flex-wrap items-center gap-1 rounded-xl bg-gray-100 dark:bg-gray-800 p-1 ${className}`}>{children}</div>;
}
...
export function TabButton({
  ...
  return (
    <button
      ...
      } ${active ? "bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 shadow-sm" : "text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200"}`}
    >
