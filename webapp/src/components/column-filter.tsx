  ...
  return (
    <div className="inline-flex items-center gap-1">
      ...
      <button
        ...
        className={`rounded p-0.5 transition-colors hover:bg-line ${isActive ? "text-indigo-600 dark:text-indigo-400" : "text-fg-faint"}`}
      >
      ...
      {mounted && open &&
        createPortal(
          <div
            ...
            className="z-50 w-56 rounded-md border border-line bg-elevated p-3 text-left font-normal normal-case tracking-normal text-fg shadow-lg"
          >
            {props.type === "text" ? (
              <input
                autoFocus
                ...
                className="w-full rounded-md border border-line-strong px-2 py-1 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              />
            ) : (
              <div className="max-h-48 space-y-1 overflow-y-auto">
                {props.options.map((o) => (
                  <label key={o.value} className="flex items-center gap-2 rounded px-1 py-0.5 text-sm hover:bg-hover">
                    <input
                      type="checkbox"
                      checked={selectedDraft.includes(o.value)}
                      onChange={() => toggleOption(o.value)}
                      className="h-3.5 w-3.5 shrink-0 rounded border-line-strong text-indigo-600 dark:text-indigo-400 focus:ring-indigo-500/40"
                    />
