    ...
    >
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          />
          <p className="mt-2 text-center text-xs text-gray-400 dark:text-gray-500">{t("Tap a point on the diagram to add a finding")}</p>
          ...
          {pendingRegion && (
            <div className="mt-3 rounded-md border border-indigo-200 dark:border-indigo-900 bg-indigo-50 dark:bg-indigo-950/40 p-3">
              ...
              <textarea
                ...
                className="w-full rounded-md border border-gray-300 dark:border-gray-700 px-2 py-1.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
              />
              <div className="mt-2 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setPendingRegion(null)}
                  className="rounded-md px-2 py-1 text-xs text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800"
                >
                  {t("Cancel")}
                </button>
                <button
        ...
        <div>
          <h3 className="mb-2 text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{t("Recorded findings")}</h3>
          {findings.length === 0 ? (
            <p className="text-sm text-gray-400 dark:text-gray-500">{t("No findings recorded yet.")}</p>
          ) : (
            <ul className="space-y-2">
              {findings.map((f, i) => (
                <li key={i} className="flex items-start justify-between gap-2 rounded-md border border-gray-200 dark:border-gray-800 p-2 text-sm">
                  <div>
                    <span className="font-medium text-gray-800 dark:text-gray-200">{t(f.region)}</span>
                    <p className="text-gray-600 dark:text-gray-400">{f.comment}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => removeFinding(i)}
                    className="shrink-0 text-xs text-gray-400 dark:text-gray-500 hover:text-red-600"
                  >
                    {t("Remove")}
                  </button>
                </li>
