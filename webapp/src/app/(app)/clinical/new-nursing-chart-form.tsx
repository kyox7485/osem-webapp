...
const fieldCls =
  "w-full rounded-md border border-line-strong px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500";
...
const selectCls =
  "w-full rounded-md border border-line-strong pl-4 pr-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500";
    ...
    <div className="space-y-4" onChangeCapture={markDirty}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Section title={t("Feeding")}>
          <div>
            <label className="mb-1 block text-sm text-fg-secondary">{t("Type")}</label>
            <select
              value={tubeFeeding}
          ...
          <div className="mt-3 space-y-2">
            <p className="text-sm font-medium text-fg-secondary">{t("Meals")}</p>
            {meals.map((meal, i) => (
              <div key={i} className="flex flex-wrap items-start gap-2">
                ...
                {meals.length > 1 && (
                  <button
                    ...
                    className="mt-2 text-xs text-fg-faint hover:text-red-600 dark:hover:text-red-300"
                  >
                    {t("Remove")}
                  </button>
                )}
  ...
  return (
    <div>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 text-sm font-medium text-fg-secondary hover:text-indigo-700 dark:hover:text-indigo-300"
      >
        <svg
          width="12"
          height="12"
...
function CheckboxGroup({
  ...
  return (
    <div>
      {label && <p className={`mb-1 text-sm text-fg-secondary ${boldLabel ? "font-bold" : "font-medium"}`}>{label}</p>}
      <div className="flex flex-wrap gap-x-3 gap-y-1">
        {options.map((o) => {
          const id = Number(o.id);
          return (
            <label key={o.id} className="flex items-center gap-1.5 text-sm text-fg-secondary">
              <input
                type="checkbox"
                checked={value.includes(id)}
                onChange={() => toggle(id)}
                className="h-4 w-4 rounded border-line-strong text-indigo-600 dark:text-indigo-400 focus:ring-indigo-500"
              />
              {o.label}
            </label>
          );
