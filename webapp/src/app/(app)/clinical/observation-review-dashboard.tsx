  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-sm space-y-3">
        <div>
          <p className="mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">{t("Period")}</p>
          <div className="flex flex-wrap gap-1">
            <button
              type="button"
              onClick={() => applyRange(3)}
              className={`rounded-md border px-3 py-2 text-sm font-medium transition-colors ${
                days === 3 && !isCustom && !showCustom ? "border-indigo-600 bg-indigo-600 text-white" : "border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:border-indigo-300 hover:text-indigo-700"
              }`}
            >
              3 {t("Days")}
            </button>
            <button
              ...
              className={`rounded-md border px-3 py-2 text-sm font-medium transition-colors ${
                days === 7 && !isCustom && !showCustom ? "border-indigo-600 bg-indigo-600 text-white" : "border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:border-indigo-300 hover:text-indigo-700"
              }`}
            >
              7 {t("Days")}
            </button>
            <button
              ...
              className={`rounded-md border px-3 py-2 text-sm font-medium transition-colors ${
                isCustom || showCustom ? "border-indigo-600 bg-indigo-600 text-white" : "border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:border-indigo-300 hover:text-indigo-700"
              }`}
            >
              {t("Custom")}
            </button>
        ...
        {showCustom && (
          <div className="flex flex-wrap items-end gap-3 pt-1">
            <div>
              ...
              <input type="date" value={customFrom} onChange={(e) => setCustomFrom(e.target.value)} className="rounded-md border border-gray-300 dark:border-gray-700 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{t("To")}</label>
              <input type="date" value={customTo} onChange={(e) => setCustomTo(e.target.value)} className="rounded-md border border-gray-300 dark:border-gray-700 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
            </div>
            <button
              type="button"
              onClick={applyCustom}
      ...
      <div className="rounded-md border border-indigo-100 bg-indigo-50 dark:bg-indigo-950/40 px-4 py-3 text-sm font-medium text-indigo-900">
      ...
      {activeEpisodes.length === 0 ? (
        ...
        <div className="space-y-3">
          {activeEpisodes.map((episode) => {
            ...
            return (
              <div key={episode.id} className="rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
                <div className="p-4">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <p className="font-bold text-gray-900 dark:text-gray-100">{residentName}</p>
                  ...
                  {!latest ? (
                    <p className="mt-3 text-sm text-gray-400 dark:text-gray-500">{t("No observation chart entries in this period.")}</p>
                  ) : (
                    <div className="mt-3 space-y-1 border-t border-gray-100 dark:border-gray-800 pt-3">
                      ...
                      <div className="grid grid-cols-1 gap-x-6 gap-y-1 text-sm text-gray-700 dark:text-gray-300 sm:grid-cols-2">
                        {comparisonRows.map((row) => (
                          <div key={row.label} className="flex justify-between gap-2">
                            <span className="text-gray-400 dark:text-gray-500">{t(row.label)}</span>
                            <span>
                              {row.previous != null && row.previous !== row.latest && (
                                <span className="text-gray-400 dark:text-gray-500 line-through decoration-gray-300">{row.previous}</span>
                              )}{" "}
                  ...
                  {residentEntries.length > 0 && (
                    <button
                      ...
                      className="mt-3 flex items-center gap-1 text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-800"
                    >
                ...
                {isExpanded && (
                  <div className="space-y-3 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/60 p-4">
                    {[...timelineByDate.entries()].map(([dateKey, dayEntries]) => (
                      <div key={dateKey}>
                        <p className="mb-1 text-xs font-semibold text-gray-500 dark:text-gray-400">{formatDate(dayEntries[0].entry_timestamp)}</p>
                        <div className="space-y-2">
                          {dayEntries.map((e) => (
                            <div key={e.id} className="rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3 text-sm shadow-sm">
                              ...
                              <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-gray-700 dark:text-gray-300 sm:grid-cols-3">
      ...
      <div className="rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
        <button
          ...
          className="flex w-full items-center gap-1 px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300"
        >
        ...
        {showCompleted && (
          <div className="space-y-2 border-t border-gray-100 dark:border-gray-800 p-4">
            {completedEpisodes.length === 0 ? (
              <p className="text-sm text-gray-400 dark:text-gray-500">{t("No completed observation episodes.")}</p>
            ) : (
              completedEpisodes.map((ep) => (
                <div key={ep.id} className="rounded-md border border-gray-200 dark:border-gray-800 p-3 text-sm">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="font-medium text-gray-900 dark:text-gray-100">{ep.tbl_residents?.resident_name ?? "--"}</span>
