...
export function NewVitalForm({ residents, allStaff, lookups, onClose, onSaved }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-2 sm:p-4" onChangeCapture={markDirty}>
      <div className="flex max-h-[calc(100dvh-1rem)] w-full max-w-3xl flex-col overflow-hidden rounded-lg bg-white dark:bg-gray-900 shadow-xl sm:max-h-[calc(100dvh-2rem)]">
        {/* Header */}
        <div className="flex flex-shrink-0 items-center justify-between border-b border-gray-200 dark:border-gray-800 px-4 py-3 sm:px-6">
          <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 sm:text-xl">{t("Record Vital Signs")}</h2>
          <button
            type="button"
            onClick={handleClose}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          <div className="px-4 py-4 sm:px-6 sm:py-6">
            {error && <div className="mb-4 rounded-md bg-red-50 dark:bg-red-950/40 p-3 text-sm text-red-800">{error}</div>}
          ...
          {dxt && (
            <div>
              <label htmlFor="insulin-adjustment" className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
        ...
        {/* Sticky Footer */}
        <div className="flex-shrink-0 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:px-6 sm:pt-4 sm:pb-[max(1rem,env(safe-area-inset-bottom))]">
