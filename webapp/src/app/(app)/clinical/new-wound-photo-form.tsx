...
export function NewWoundPhotoForm({ residents, allStaff, bodyParts, presetResidentId, onSaved }: Props) {
  ...
  const uploadMoreButtonClass =
    "flex items-center justify-center gap-2 rounded-lg border-2 border-indigo-600 bg-surface px-6 py-3.5 text-base font-semibold text-indigo-700 dark:text-indigo-300 shadow-sm hover:bg-indigo-50 dark:hover:bg-indigo-950/40";
    ...
    <div className="space-y-4">
      ...
      {!residentId ? (
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {unmappedBodyParts.map((part) => (
                <button
                  ...
                  className="rounded-full border border-line-strong bg-surface px-3 py-1.5 text-sm text-fg-secondary hover:border-indigo-400"
                >
                  {t(part.label)}
          ...
          {photos.length > 0 && (
            <div className="flex justify-center border-t border-line-subtle pt-4">
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-bold text-fg">{t(activePart!.label)}</h3>
          ...
          {stage === "idle" && (
            <button
              type="button"
              onClick={openCamera}
              className="flex w-full items-center justify-center gap-2 rounded-md border-2 border-dashed border-indigo-300 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-950/40 py-8 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/40"
            >
          ...
          {stage === "previewing" && rawPreviewUrl && (
            <div className="space-y-3">
              ...
              <img src={rawPreviewUrl} alt={t("Photo preview")} className="mx-auto max-h-96 rounded-md border border-line" />
              <div className="flex justify-center gap-3">
                <button type="button" onClick={handleRetake} className="flex items-center gap-1 rounded-md border border-line-strong px-4 py-2 text-sm">
          ...
          {stage === "compressing" && <p className="text-center text-sm text-fg-faint">{t("Processing photo...")}</p>}

          <div>
            <label className="mb-1 block text-sm font-medium text-fg-secondary">
              {t("Description")} <span className="text-fg-faint">({t("optional")})</span>
            </label>
            <textarea
              value={description}
            ...
            />
            <p className="mt-1 text-xs text-fg-faint">{t("Applies to the next photo you take here.")}</p>
          </div>
          ...
          {photosForActivePart.length > 0 && (
            <div className="grid grid-cols-2 gap-3 border-t border-line-subtle pt-4 sm:grid-cols-3">
              {photosForActivePart.map((p) => (
                <div key={p.clientId} className="relative overflow-hidden rounded-md border border-line">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <div className="flex flex-col items-center justify-center gap-3 border-t border-line-subtle pt-6 sm:flex-row">
