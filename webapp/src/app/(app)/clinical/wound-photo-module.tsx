          ...
          <div className="space-y-3">
            {sessions.length === 0 ? (
              ...
              sessions.map((session) => (
                <div key={session.id} className="rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-sm">
                  <div className="mb-3 flex items-center justify-between">
                    <span className="font-bold text-gray-900 dark:text-gray-100">{session.tbl_residents?.resident_name}</span>
                    <span className="text-xs text-gray-400 dark:text-gray-500">{formatDateTime(session.session_started_at)}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    {session.photos.map((photo) => (
                      <div key={photo.id} className="overflow-hidden rounded-md border border-gray-200 dark:border-gray-800">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={`/api/wound-photos/${photo.id}`} alt={photo.body_part_label} className="h-32 w-full object-cover" loading="lazy" />
                        <div className="p-2">
                          <p className="text-xs font-medium text-gray-900 dark:text-gray-100">{t(photo.body_part_label)}</p>
                          {photo.description && <p className="text-xs text-gray-500 dark:text-gray-400">{photo.description}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
