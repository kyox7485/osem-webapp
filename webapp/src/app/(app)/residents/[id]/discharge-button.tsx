...
export function DischargeButton({
  ...
  const inputCls =
    "w-full rounded-md border border-line-strong bg-input px-3 py-2 text-sm text-fg focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20";
    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          >
            <h2 className="text-base font-semibold text-fg">{t("Discharge resident")}</h2>
            <p className="mt-1 text-sm text-fg-subtle">{t("Complete the details below to record the discharge.")}</p>

            <form action={handleSubmit} className="mt-5 space-y-4">
              <div>
                <label className="block text-xs font-medium text-fg-muted mb-1">{t("Status")}</label>
