...
import { FamilyReminderDocument } from "@/lib/pdf/documents/family-reminder-document";
...
// Family Medicine Reminder PDF: family-supplied active orders, grouped by
...
export async function GET(request: NextRequest) {
  ...
  const now = new Date();
  const result = await buildStockReport(supabase, resident.id, "Family", now);
  if ("error" in result) return NextResponse.json({ error: result.error }, { status: 500 });
  const { report } = result;

  if (report.orderCount === 0) {
    const { t } = await getServerTranslator();
    return nothingToGenerateResponse(t("Nothing to generate: this resident has no active family-supplied medicine."));
  }

  registerCjkFont();
  const branch = await getReportBranchInfo(resident.branch_id);
  const buffer = await renderToBuffer(
    <FamilyReminderDocument
  ...
  return new NextResponse(buffer as unknown as BodyInit, {
    ...
    headers: {
      ...
      "Content-Disposition": `inline; filename="family-reminder-${resident.ResidentID ?? residentId}.pdf"`,
